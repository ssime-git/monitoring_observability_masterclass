...
async def load_user(username):
    ...
    try:
        ...
        logger.error(f"Redis error in load_user: {str(e)}")
    
    # If Redis fails or user not in cache, try to get from allowed_users
    for user_data in allowed_users:
        if user_data['username'] == username:
            logger.debug(f"user {username} found in allowed_users")
            try:
                await cache.set(username, user_data, ttl=3600)  # Try to cache for next time
            except Exception as e:
...
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    try:
        await cache.set(form_data.username, user_data, ttl=3600)  # Cache for 1 hour
    except Exception as e:
        logger.error(f"Redis cache set error in login: {str(e)}")
